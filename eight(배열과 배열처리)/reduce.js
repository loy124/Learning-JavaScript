...
// 첫번째 매개변 수는 배열이 줄어드는 대상인 acculmulator, 두번쨰 매개변수부터는 배열요소, 현재 인덱스, 배열자체다
//forEach: for문을 돌리는것과 비슷
//map : for문을 돌려서 새로운 배열을 만드는 목적 return 필수

//reduce: for문을 돌려서 최종적으로 다른 무언가를 만드는 목적 return필수
//배열의 숫자를 더하는 예제(누적값)
const arr = [5, 7, 2, 4];
...
//객체 만들기
cosnt arr = ['a','b', 'c', 'd']
const str = arr,reduce(function (res, item, indx, array) {
    res[item] = item;
    return res;
}, {})
console.log(str)
